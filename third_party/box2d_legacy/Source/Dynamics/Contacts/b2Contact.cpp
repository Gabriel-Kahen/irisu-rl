...
namespace
{
// IRISU SAFETY PATCH: the process-wide contact factory table is immutable after
// this one-time initialization and can then be read by independent worlds.
std::once_flag b2_contactRegistersOnce;
}
...
{
	std::call_once(b2_contactRegistersOnce, [] {
		InitializeRegisters();
		s_initialized = true;
	});
