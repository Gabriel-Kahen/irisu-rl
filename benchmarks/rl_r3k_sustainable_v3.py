#!/usr/bin/env python3
"""Fresh development-only matched G4 versus R3K long-horizon screen v3.
...
V3 retains the fail-closed rule discovered by the burned v1 campaign: if the
frozen incumbent is absent from the legal joint inventory, the query abstains
and executes the unchanged incumbent.  It uses the ledger-aware v3 runway
authority and requires a separate intervention-viability report before init.
"""
...
from irisu_pointer import resolution_sustainable_r3k_v3 as r3k  # noqa: E402


RUN_ID = "r3k-runway-long-screen-20260808-007"
...
VIABILITY_REPORT = (
    ROOT
    / "artifacts/r3/development/r3k-runway-long-screen-20260808-007-prelaunch/"
    "viability-report.json"
)
COLLISION_PROOF = VIABILITY_REPORT.with_name("seed-collision-proof.json")
...
def _source_files() -> tuple[Path, ...]:
    return (
        ...
        ROOT / "python/irisu_pointer/resolution_sustainable_r3k_v3.py",
        ROOT / "tests/test_pointer_resolution_sustainable_r3k_v3.py",
        ROOT / "tests/test_r3k_sustainable_runner_v3.py",
        ROOT / "benchmarks/r3k_v3_prelaunch.py",
        VIABILITY_REPORT,
        COLLISION_PROOF,
        *EXPECTED,
    )
...
def initialize(run_root: Path) -> dict[str, object]:
    ...
    viability = _read_json(VIABILITY_REPORT)
    viability_payload = dict(viability)
    viability_sha = viability_payload.pop("sha256", None)
    if (
        viability.get("schema") != "irisu-r3k-v3-intervention-viability-v1"
        or viability.get("passed") is not True
        or viability.get("sealed_test_allowed") is not False
        or viability_sha != _canonical_sha256(viability_payload)
        or int(viability.get("queries", 0)) < 24
        or int(viability.get("complete_units", 0)) < 16
        or int(viability.get("viable_candidates", 0)) < 16
        or int(viability.get("selected_alternatives", 0)) < 8
        or int(viability.get("selected_rescue", 0)) < 2
        or int(viability.get("selected_growth", 0)) < 4
        or int(viability.get("intervention_units", 0)) < 8
        or int(viability.get("strict_survival_rescues", 0)) < 2
        or int(viability.get("nonzero_clear_targets", 0)) < 1
        or int(viability.get("unsafe_selected", -1)) != 0
        or int(viability.get("lost_selected", -1)) != 0
        or int(viability.get("terminal_selected", -1)) != 0
        or int(viability.get("invalid_selected", -1)) != 0
        or int(viability.get("rebind_failures", -1)) != 0
        or int(viability.get("restore_failures", -1)) != 0
        or int(viability.get("selected_rescue", 0))
        + int(viability.get("selected_growth", 0))
        != int(viability.get("selected_alternatives", -1))
        or int(viability.get("query_artifacts", 0))
        != int(viability.get("queries", -1))
    ):
        raise RuntimeError("R3K v3 intervention viability gate did not pass")
    viability_seeds = tuple(int(value) for value in viability["seeds"])
    if set(viability_seeds) & set(SEEDS):
        raise RuntimeError("R3K v3 viability and screen seeds overlap")
    expected_viability_sources = {
        str(Path(__file__).resolve()): _sha256_file(Path(__file__).resolve()),
        str(Path(r3k.__file__).resolve()): _sha256_file(Path(r3k.__file__).resolve()),
        str(ROOT / "benchmarks/r3k_v3_prelaunch.py"): _sha256_file(
            ROOT / "benchmarks/r3k_v3_prelaunch.py"
        ),
    }
    if viability.get("source_sha256") != expected_viability_sources:
        raise RuntimeError("R3K v3 viability source identity differs")
    collision = _read_json(COLLISION_PROOF)
    collision_payload = dict(collision)
    collision_sha = collision_payload.pop("sha256", None)
    if (
        collision.get("schema") != "irisu-r3k-v3-seed-collision-proof-v1"
        or collision_sha != _canonical_sha256(collision_payload)
        or collision.get("collisions") != []
        or tuple(int(value) for value in collision.get("screen_seeds", ())) != SEEDS
        or tuple(int(value) for value in collision.get("viability_seeds", ()))
        != viability_seeds
    ):
        raise RuntimeError("R3K v3 seed collision proof did not pass")
    rows = viability.get("rows", ())
    if type(rows) is not list or len(rows) != int(viability["queries"]):
        raise RuntimeError("R3K v3 viability row closure differs")
    for row in rows:
        path = VIABILITY_REPORT.parent / str(row["query_artifact"])
        artifact = _read_json(path)
        artifact_payload = dict(artifact)
        artifact_sha = artifact_payload.pop("sha256", None)
        if (
            artifact_sha != _canonical_sha256(artifact_payload)
            or artifact_sha != row.get("query_artifact_sha256")
            or artifact.get("seed") != row.get("seed")
            or artifact.get("query_index") != row.get("query_index")
            or _canonical_sha256(artifact.get("query")) != row.get("query_sha256")
        ):
            raise RuntimeError("R3K v3 viability query artifact differs")
    identity = _source_identity()
    preregistration = _with_sha(
        {
            "schema": "irisu-r3k-long-screen-preregistration-v3",
            ...
            "extension_budget": r3k.EXTENSION_BUDGET,
            "viability_report_sha256": viability_sha,
            "seed_collision_proof_sha256": collision_sha,
            ...
            "selection": {
                ...
                "long_safety": (
                    "RenewableDebtLedger cashflow loss or GAME_OVER; NumPy public "
                    "scalars converted exactly; no minimum-gauge proxy"
                ),
            },
            "promising_gates": {
    ...
    *,
    ...
    if selected_identity.ordinal:
        ...
        if rebound is not True:
            raise RuntimeError("R3K selected alternative failed live rebind")
    selected_short = next(
