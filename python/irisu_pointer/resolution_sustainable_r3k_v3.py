...
R3K_SCHEMA = "irisu-r3k-sustainable-runway-v3"
SHORT_HORIZON = 768
...
def _public_count(observation: Mapping[str, Any], name: str) -> int:
    if name not in observation:
        raise KeyError(f"R3K public field {name!r} is absent")
    value = observation[name]
    item = getattr(value, "item", None)
    if callable(item):
        value = item()
    if type(value) is not int:
        raise TypeError(f"R3K public field {name!r} is not an exact integer")
    return value
...
class RunwayCheckpoint:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            or type(self.active_due_ids) is not tuple
            or tuple(sorted(set(self.active_due_ids))) != self.active_due_ids
            or any(type(value) is not int or value < 0 for value in self.active_due_ids)
        ):
    ...
    def manifest(self) -> dict[str, object]:
        return {
            ...
            "level": self.level,
            "active_due_ids": list(self.active_due_ids),
        }
...
class RunwayOutcome:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            or (
                self.cashflow_lost_tick is not None
                and (
                    type(self.cashflow_lost_tick) is not int
                    or self.cashflow_lost_tick < 1
                )
            )
            or self.cashflow_lost != (self.cashflow_lost_tick is not None)
            or self.terminal
            != (self.terminated or self.truncated or self.game_over or self.cashflow_lost)
            or self.horizon_exhausted
            != (self.survival_ticks == CHECKPOINT_HORIZONS[-1])
            or type(self.cashflow_sha256) is not str
            or len(self.cashflow_sha256) != 64
            or type(self.renewal_ticks) is not tuple
            or tuple(sorted(set(self.renewal_ticks))) != self.renewal_ticks
            or any(type(value) is not int or value < 1 for value in self.renewal_ticks)
            or type(self.unresolved) is not tuple
            or any(type(value) is not str or not value for value in self.unresolved)
        ):
    ...
    def manifest(self) -> dict[str, object]:
        ...
        return {
            "schema": "irisu-r3k-runway-outcome-v3",
            "identity": identity,
            ...
            "cashflow_lost": self.cashflow_lost,
            "cashflow_lost_tick": self.cashflow_lost_tick,
            "cashflow_sha256": self.cashflow_sha256,
...
def rollout_candidate(
    ...
    *,
    ...
    start_clears = _public_count(initial, "qualifying_clear_count")
    current = initial
    ...
    invalid_actions = 0
    game_over = False
    ledger = core.RenewableDebtLedger(initial)
    checkpoints: dict[int, RunwayCheckpoint] = {}
    ...
    def capture(horizon: int) -> RunwayCheckpoint:
        ...
        active_due = tuple(
            sorted(
                int(body_id)
                for body_id, liability in ledger.liabilities.items()
                if liability.status == "active" and liability.due_tick <= elapsed_now
            )
        )
        return RunwayCheckpoint(
            ...
            _public_count(current, "qualifying_clear_count") - start_clears,
            _public_count(current, "gauge"),
            int(ledger.minimum_public_gauge),
    ...
    def unit_step(action: Any) -> None:
        ...
        for _ in range(duration):
            if terminated or truncated or ledger.lost or game_over:
                break
            primitive = (
            ...
            ledger.advance(previous, current, events)
            relative_tick = int(current["tick"]) - start_tick
            if relative_tick in CHECKPOINT_HORIZONS:
                checkpoints[relative_tick] = capture(relative_tick)
    ...
    if not (terminated or truncated or ledger.lost) and elapsed < cooldown:
    ...
    if not rebound:
        ledger.unresolved.append("continuation-rebind-failed")

    # This is one continuous trajectory.  The 128/512 rows are read-only
    # observations captured inside a macro; they never clip a macro or cause
    # an extra policy prediction.  Only the final 2,048 cap may clip waits.
    final_horizon = CHECKPOINT_HORIZONS[-1]
    while rebound and not (terminated or truncated or ledger.lost or game_over):
        elapsed = int(current["tick"]) - start_tick
        if elapsed >= final_horizon:
            break
        decision = policy.predict(current)
        actions = core.ExactBranchEvaluator._primitive_tuple(decision, resolved_spec)
        if not actions:
            ledger.unresolved.append("empty-continuation-macro")
            break
        remaining = final_horizon - elapsed
        expanded = sum(
            int(action.wait_ticks)
            if core.JOINT.ActionKind.parse(action.kind) is core.JOINT.ActionKind.WAIT
            else 1
            for action in actions
        )
        if expanded > remaining and any(
            core.JOINT.ActionKind.parse(action.kind) is not core.JOINT.ActionKind.WAIT
            for action in actions
        ):
            ledger.unresolved.append("nonwait-macro-crossed-final-horizon")
            break
        for action in actions:
            remaining = final_horizon - (int(current["tick"]) - start_tick)
            if remaining <= 0 or terminated or truncated or ledger.lost or game_over:
                break
            kind = core.JOINT.ActionKind.parse(action.kind)
            duration = int(action.wait_ticks) if kind is core.JOINT.ActionKind.WAIT else 1
    ...
    checkpoint_rows = tuple(
        checkpoints.get(horizon, capture(horizon)) for horizon in CHECKPOINT_HORIZONS
    )

    ledger_result = ledger.finalize(CHECKPOINT_HORIZONS[-1])
    return RunwayOutcome(
        ...
        bool(game_over),
        min(int(current["tick"]) - start_tick, CHECKPOINT_HORIZONS[-1])
        == CHECKPOINT_HORIZONS[-1],
        bool(terminated or truncated or game_over or ledger.lost),
        bool(game_over or ledger.lost),
        invalid_actions,
        not rebound,
        bool(ledger_result["cashflow_lost"]),
        ledger_result["cashflow_lost_tick"],
        str(ledger_result["cashflow_sha256"]),
        tuple(int(value) for value in ledger_result["renewal_ticks"]),
        tuple(str(value) for value in ledger_result["unresolved"]),
    )
...
def extension_identities(
    ...
    """Return incumbent plus a four-view deterministic safe union."""
    ...
    if mode not in {"rescue", "growth"}:
        raise ValueError("R3K proposal mode is malformed")
    ...
    def solvency_key(row: object) -> tuple[float, float, int]:
        prediction = prediction_by_identity.get(row.identity)
        conservative = (
            -math.inf
            if prediction is None
            else float(prediction.solvency_mean) - float(prediction.solvency_std)
        )
        return (-conservative, -float(row.b2), row.identity.ordinal)

    growth_rank = sorted(eligible, key=growth_key)
    solvency_rank = sorted(eligible, key=solvency_key)
    selected = [outcomes[0].identity]
    ranks = (b2_rank, growth_rank) if mode == "rescue" else (solvency_rank, growth_rank)
    for rank in ranks:
        for row in rank[:2]:
            if row.identity not in selected:
                selected.append(row.identity)
                if len(selected) - 1 == min(EXTENSION_BUDGET, len(eligible)):
                    return tuple(selected)
    cursors = [0] * len(ranks)
    while len(selected) - 1 < min(EXTENSION_BUDGET, len(eligible)):
        before = len(selected)
        for index, rank in enumerate(ranks):
            while cursors[index] < len(rank):
                identity = rank[cursors[index]].identity
                cursors[index] += 1
        ...
        if len(selected) == before:
            raise RuntimeError("R3K safe extension union could not fill its budget")
    return tuple(selected)
...
def select_sustainable(
    ...
    for outcome in short[1:]:
        ...
        if (
            ...
            or runway.terminal
            or runway.cashflow_lost
            or bool(runway.unresolved)
            or runway.invalid_actions
            or runway.gauge_failure
            or runway.survival_ticks != CHECKPOINT_HORIZONS[-1]
        ...
        if (
            mode == "rescue"
            and runway.survival_ticks == incumbent_long.survival_ticks
            and (
                incumbent_short.b2 is None
