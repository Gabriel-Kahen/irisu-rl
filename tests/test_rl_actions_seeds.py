...
class ActionAndSeedTests(unittest.TestCase):
    def test_codec_validates_before_lowering_and_never_emits_both(self) -> None:
        ...
        self.assertEqual(spec.press(SemanticAction.weak(0.5, 0.25)).kind, ActionKind.WEAK_SHOT)
        ...
        for action in (SemanticAction.wait(37), SemanticAction.weak(0.2, 0.8), SemanticAction.strong(1.0, 0.0)):
    ...
    def test_conditional_log_prob_ignores_inactive_heads(self) -> None:
        ...
        baseline = ConditionalActionDistribution(kind, wait, alpha, beta).log_prob(actions)
        changed = alpha.copy()
        changed[0] = 100.0  # wait row has no active coordinate head
        changed[1, 0] = 100.0  # strong row has no active weak head
        actual = ConditionalActionDistribution(kind, wait, changed, beta).log_prob(actions)
    ...
    def test_distribution_sampling_is_reproducible_and_finite(self) -> None:
        distribution = ConditionalActionDistribution(
            np.zeros((16, 3)), np.zeros((16, 100)), np.full((16, 2, 2), 2.0), np.full((16, 2, 2), 2.0)
        )
    ...
    def test_masks_and_deterministic_evaluation(self) -> None:
        ...
        wait[0, 36] = 5
        distribution = ConditionalActionDistribution(
            np.array([[9.0, 8.0, 7.0]]), wait,
            np.full((1, 2, 2), 2.0), np.full((1, 2, 2), 2.0),
    ...
    def test_checked_in_action_seed_reward_and_runtime_configs_match_code(self) -> None:
        ...
        self.assertEqual(runtime["worker_sha256"], ACCEPTED_EXACT_RUNTIME_2026_07_21.worker_sha256)
        self.assertEqual(runtime["exact_library_sha256"], ACCEPTED_EXACT_RUNTIME_2026_07_21.exact_library_sha256)
