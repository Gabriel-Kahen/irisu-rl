...
class SchemaEncodingTests(unittest.TestCase):
    def test_schema_identities_are_stable_and_actor_excludes_privileged_fields(self) -> None:
        ...
        for schema in (ACTOR_VISION_V1, TEACHER_V1):
            manifest = json.loads(
                (root / "configs" / "rl" / "schemas" / f"{schema.version}.json").read_text()
            )
    ...
    def test_velocity_conversion_and_shape_symmetry(self) -> None:
        ...
        self.assertAlmostEqual(rows[0, names.index("velocity_x_display_per_second_scaled")], 0.1)
        self.assertAlmostEqual(rows[1, names.index("velocity_x_display_per_second_scaled")], 0.2)
        self.assertEqual(rows[1, names.index("orientation_valid")], 0.0)

    def test_actor_requires_causal_records_and_selects_overflow_deterministically(self) -> None:
    ...
    def test_actor_requires_effect_time_units_and_excludes_fully_occluded_tracks(self) -> None:
        encoder = ActorTrackEncoder()
        ...
        self.assertEqual(encoded.body_features[0, 0, names.index("orientation_valid")], 0)
        tied = [
    ...
    def test_explicit_teacher_display_velocity_is_not_converted_twice(self) -> None:
        ...
        self.assertAlmostEqual(
            encoded.body_features[0, 0, names.index("velocity_x_display_per_second_scaled")],
            0.1,
        )
